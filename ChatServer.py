    ...
    def __init__(self, options, run_on_localhost=False):
        ...
        * self.request_terminate (bool): a flag used by the testing application to indicate whether your code
        ...
        self.message_handlers = {
            ...
            0x02: self.handle_server_quit_message,
...
##############################################################################################################
    ...
    def setup_server_socket(self):
        ...
        try:
            ...
            self.print_info(f"Error binding to port {self.port}: {e}")
            return
        
        server_sock.listen(10)
        self.sel.register(server_sock, selectors.EVENT_READ, None)
    ...
    def connect_to_server(self):
        ...
        conn_data = ServerConnectionData(self.id, self.server_name, self.server_info)
        
        conn_data.write_buffer = ServerRegistrationMessage.bytes(
            self.id, 0, self.server_name, self.server_info
        )
        
        events = selectors.EVENT_READ | selectors.EVENT_WRITE
        self.sel.register(sock, events, conn_data)
    ...
    def cleanup(self):
        ...
        # Use public get_map() for cleaner retrieval
    ...
    def accept_new_connection(self, io_device):
        ...
        """
        # TODO: Implement the above functionality
        conn, _ = io_device.fileobj.accept()
        conn.setblocking(False)
        conn_data = BaseConnectionData()
        # Register to handle both read and write
        self.sel.register(conn, selectors.EVENT_READ | selectors.EVENT_WRITE, conn_data)
...
##############################################################################################################
    ...
    def broadcast_message_to_servers(self, message, ignore_host_id=None):
        ...
        """
        # TODO: Implement the above functionality
        for sid in self.adjacent_server_ids:
            if sid == ignore_host_id:
                continue
            
            self.send_message_to_host(sid, message)
    ...
    def broadcast_message_to_adjacent_clients(self, message, ignore_host_id=None):
        ...
        """
        # TODO: Implement the above functionality
        for cid in self.adjacent_user_ids:
            if cid ==  ignore_host_id:
                continue
            
            self.send_message_to_host(cid, message)
...
##############################################################################################################
    ...
    def handle_server_registration_message(self, io_device, message):
        ...
        if message.source_id in self.hosts_db:
            status = StatusUpdateMessage.bytes(self.id, 0, 0x02, "A machine has already registered with ID " + str(message.source_id))
            io_device.data.write_buffer += status
            return
...
##############################################################################################################
    ...
    def handle_client_registration_message(self, io_device, message):
        ...
        # TODO: Implement the above functionality
        if message.source_id in self.hosts_db:
            status = StatusUpdateMessage.bytes(self.id, 0, 0x02, "Someone has already registered with ID " + str(message.source_id))
            io_device.data.write_buffer +=status
            return
        ...
        if message.last_hop_id == 0:
            firstmessage = StatusUpdateMessage.bytes(self.id, message.source_id, 0x00, "Welcome to the Clemson Relay Chat network " + str(message.client_name))
            client_data.write_buffer += firstmessage
...
##############################################################################################################
    ...
    def handle_status_message(self, io_device, message):
        ...
        if message.destination_id in (self.id, 0):
...
##############################################################################################################
    ...
    def handle_client_chat_message(self, io_device, message):
        ...
        """
        # TODO: Implement the above functionality
        # Use get() and early return for unknown destination
        dest = self.hosts_db.get(message.destination_id)
        
        if not dest:
            unknown_msg = StatusUpdateMessage.bytes(self.id, message.source_id, 0x01, f"Unknown ID {message.destination_id}")
            self.send_message_to_unknown_io_device(io_device, unknown_msg)
            return
        
        chat_bytes = ClientChatMessage.bytes(message.source_id, message.destination_id, message.content)
        self.send_message_to_host(dest.first_link_id, chat_bytes)
...
##############################################################################################################
    ...
    def handle_server_quit_message(self, key, message):
        """Process a ServerQuitMessage: terminate server when this node is closing."""
        # message.replacement_id indicates new server; close this server
